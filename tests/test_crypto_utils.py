        ...
        for i_1 in range(i_2, len(input_string)):
            ...
            for hash_fn_1 in crypto_utils.HashFunction:
                ...
                if hash_fn_1 == crypto_utils.HashFunction.SHA256:
                    assert len(hash_1) == 64, f"Received SHA256 hash {hash_1}, expected hex rep to have length 64, received {len(hash_1)}"
                elif hash_fn_1 == crypto_utils.HashFunction.RIPEMD160:
                    assert len(hash_1) == 40, f"Received RIPEMD160 hash {hash_1}, expected hex rep to have length 40, received {len(hash_1)}"
                    ...
                    for j_1 in range(j_2, len(input_string)):
                        ...
                        for hash_fn_2 in crypto_utils.HashFunction:
                            ...
                            if msg_1 == msg_2 and hash_fn_1 == hash_fn_2:
                                assert hash_1 == hash_2, f"Expected hashes {hash_1} and {hash_2} to have equal values with inputs {msg_1} and {msg_1} and hash function {hash_fn_1} and {hash_fn_2}"
                            else:
                                assert hash_1 != hash_2, f"Expected hashes {hash_1} and {hash_2} to have unequal values with inputs {msg_1} and {msg_1} and hash function {hash_fn_1} and {hash_fn_2}"
...
def test_validate_zeroes():
    ...
    for i in range(6):
        assert crypto_utils.validate_zeroes(str_1, i), f"Failed to validate with values {str_1} for {i} zeroes"

    for i in range(9):
        assert crypto_utils.validate_zeroes(str_2, i), f"Failed to validate with values {str_2} for {i} zeroes"

    for i in range(12):
        assert crypto_utils.validate_zeroes(str_3, i), f"Failed to validate with values {str_3} for {i} zeroes"

    assert not crypto_utils.validate_zeroes(str_1, 6), f"Erroneously validated for values {str_1} and 6 zeroes."
    assert not crypto_utils.validate_zeroes(str_1, 9), f"Erroneously validated for values {str_2} and 9 zeroes."
    assert not crypto_utils.validate_zeroes(str_1, 12), f"Erroneously validated for values {str_3} and 12 zeroes."
...
def test_signature_functions():
    ...
    valid_sig = crypto_utils.validate_signature(message=message, signature=alice_signature, pub_key=alice_pub)
    assert valid_sig, f"Failed to validate signature over message {message} with public key {alice_pub}"
    bad_validation = crypto_utils.validate_signature(message=message, signature=alice_signature, pub_key=eve_pub)
    assert not bad_validation, f"Successfully validated message {message} with pub key {eve_pub}, which should have required pub key {alice_pub}"
    eve_spoofed_sig = crypto_utils.sign_message(message=message, private_key=eve_priv) #Should this throw an error?
    attempted_spoof = crypto_utils.validate_signature(message=message, signature=eve_spoofed_sig, pub_key=alice_pub)
    assert not attempted_spoof, f"Spoofed signature of message {message} with private key {eve_priv} evaluated as valid. Should have required private key {alice_priv}"
    wrong_validation = crypto_utils.validate_signature(message=wrong_message, signature=alice_signature, pub_key=alice_pub)
    assert not wrong_validation, f"Successfully validated message {wrong_message} with signature which should have been over {message}"
...
def test_compare_hashes():
    ...
    bytes_2 = int.to_bytes(int("001001001001110110110110",2), length=3) #Every 3rd digit should be different
    hash_1 = binascii.hexlify(bytes_1).decode(encoding="utf-8")
    hash_2 = binascii.hexlify(bytes_2).decode(encoding="utf-8")
    assert len(hash_1) == 6, print(f"Test hash had length {len(hash_1)}, expected 6") #Make sure our encoding worked as expected, otherwise 
    assert len(hash_2) == 6, print(f"Test hash had length {len(hash_2)}, expected 6") #nothing else will work
