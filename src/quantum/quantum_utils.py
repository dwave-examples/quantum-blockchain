...
def uint64_to_bitarray(stats_int: np.ndarray) -> np.ndarray:
    ...
    """
    stats_int =  np.flip(np.reshape(stats_int.view(np.uint8),
                                    (len(stats_int), 8)), axis=1)
...
def default_ising_model(lattice_size: int=8, random_seed: int=None, verbose: bool=False):  # use_qpu is for now a global variable
    ...
    J = {((x, y), ((x + disp[0])%lattice_size, (y + disp[1])%lattice_size)): 1-2*prng.random()
        for x in range(lattice_size) for y in range(lattice_size) for disp in [(0,1), (1,0)] }
    # h = {(x,y): 0 for x in range(lattice_size) for y in range(lattice_size)}  # h is assumed ordered, python 3.10
    h = {(x,y): 2*(prng.random()-.5) for x in range(lattice_size) for y in range(lattice_size)} 
...
def generate_default_sampler(J: dict,
                    num_reads: int=10000,
                    use_qpu: bool=False,
                    verbose: bool=False,
                    seed: int=None,
    ...
    if use_qpu:
        ...
        if verbose:
            print('Searching for a subgraph, this can be slow '
        ...
        sampler_kwargs = dict(
            ...
            fast_anneal=True, annealing_time=0.005,  # 5 nanosecs.
            auto_scale=False)
    else:
        if verbose:
            print('Placeholder SA sampler has been tuned to look'
        ...
        sampler_kwargs = {'beta_range': [1/np.sqrt(
            np.sum([Jij**2 for Jij in J.values()])), beta_max],
                        'num_reads': num_reads,
...
def bit_correlation(bit_array: np.ndarray) -> np.ndarray:
    ...
    mean_correlation = (correlation_matrix.sum(1)-np.diag(correlation_matrix))/(correlation_matrix.shape[1]-1)
...
def build_stats(samples:np.ndarray,
                J: dict,
                h: np.ndarray,
                bound: bool=True,
    ...
    corrs = np.einsum('si,sj->ij', samples, samples)/samples.shape[0] # sample values should be floats
...
if __name__ == '__main__':
    ...
    sampler, sampler_kwargs = generate_default_sampler(JJ, num_reads=1000, use_qpu=False, verbose=True, profile='defaults')
    sampler_kwargs['J'] = JJ
    sampler_kwargs['h'] = hh
    sampler_output = sampler.sample_ising(**sampler_kwargs)
    import dimod
    energies = [dimod.utilities.ising_energy(sample, hh, JJ) for sample in sampler_output.record.sample]
