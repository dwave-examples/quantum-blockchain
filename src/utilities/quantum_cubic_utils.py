...
def get_energy_scale(solver, adv_fudge_factor=1):
    ...
    if solver == "Advantage2_prototype2.6" or solver == "Advantage2_prototype2_x_internal":
        return 1
...
def generator_shuffle(i_to_map, generators, seed=None, num_shuffles=4):  # Yes its fast mixing
    prng = np.random.default_rng(seed)
...
class ParallelEmbeddingsComposite(dimod.Sampler, dimod.Structured):  # dimod.ComposedSampler):
    ...
    def sample(self, bqm, randomize_embedding=True, num_programmings=1, **kwargs):
        ...
        try:
            ...
            __, __, target_adjacency = self.child.child.structure  # SpinReversalComposite problem!
        if randomize_embedding:
            _embs = [shuffle_embedding(emb, generators=self.generators) for emb in self.embeddings]
        else:
            _embs = self.embeddings
        for embedding in _embs:
            embedding = {
                tuple(int(v) for v in source): tuple(int(v) for v in target)
...
def dimerize_coupling_3d(n1, n2, z_parity, modulo):
    ...
    if (n2[0] - n1[0]) % modulo[0] == 1 or (n2[0] - n1[0]) % modulo[0] == modulo[0] - 1:
        d1 = d2 = 0
    elif (n2[1] - n1[1]) % modulo[1] == 1 or (n2[1] - n1[1]) % modulo[1] == modulo[1] - 1:
        d1 = d2 = 1
...
def create_dimerizedbiclique(dimensions=18):
    ...
    if dimensions < 24:
        ...
        edge_list = [(relabel[u], relabel[v]) for u, v in edge_list if u in spins and v in spins]
...
def create_lattice(Ls=[10, 10], is_periodic=[False, False], dimerize_lattice=None):
    ...
    if dimerize_lattice is not False:
        ...
        edgeset |= {dimerize_coupling_3d(n1, n2, z_parity=0, modulo=modulo) for n1, n2 in edgelist}
        if dimerize_lattice == "doubled":
            edgeset |= {
                dimerize_coupling_3d(n1, n2, z_parity=1, modulo=modulo) for n1, n2 in edgelist
            }
...
def create_model(ensemble, seed, Ls=[4, 4, 4], is_periodic=[False, False, True], dimensions=18):
    """Create supremacy paper model"""
    prng = np.random.default_rng(seed)
    if ensemble == "DimBiClique":
        nodelist, edgelist = create_dimerizedbiclique(dimensions=dimensions)
        tupledict = {v: (v // 36, (v % 36) // 2, (v % 36) % 6 // 2, v % 2) for v in nodelist}
        ...
        J = {
            ij: (
                2 * prng.integers(2) - 1 if ij[0][2] != ij[1][2] else (2 * prng.integers(2) - 1) / 2
            )
            for ij in edgelist
        }
        ...
        nodelist, edgelist = create_lattice(Ls, is_periodic, dimerize_lattice="doubled")
        J = {
            ij: (2 * prng.random() - 1 if ij[0][2] != ij[1][2] else (2 * prng.random() - 1) / 2)
            for ij in edgelist
        }
        J.update({ij: -2 for ij in edgelist if ij[0][:3] == ij[1][:3]})
    h = {i: 0 for i in nodelist}
...
def get_max_num_reads(qpu, qpu_kwargs, num_var=None, max_time=float("Inf")):
    ...
    constant_time, per_read_time = get_qpu_access_times(qpu, qpu_kwargs, num_var=num_var)
...
def generate_default_sampler(
    ...
    if use_qpu:
        ...
        if len(embeddings) > 0:
            ...
            raise ValueError("Embeddings not found, using find_subgraph (should not happen?)")
...
def unit_test_basic():
    ...
    print(f"#embs={len(embs)} x #reads={num_reads} = {np.sum(ss.record.num_occurrences)}")
...
def unit_test_functional_BER(
    ...
    if stat_type == "NN":
        ...
        stat_edges = [
            (int(n1), int(n2)) for idx, n1 in enumerate(nodelist) for n2 in nodelist[idx:]
        ]
    ...
    for seed in range(num_tests):
        ...
        for solver in solvers:
            ...
            sampler, sampler_kwargs = generate_default_sampler(**kwargs, qpu=qpus[solver])
