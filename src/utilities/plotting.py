...
def common_predecessors(directed_tree: nx.DiGraph, *, nodes: str) -> tuple[set, set]:
...
def plot_graph(
    G: Union[nx.Graph, nx.DiGraph], save_as: str, labels=None, active_blocks: dict = None
) -> None:
    ...
    pos = nx.spiral_layout(
        G.nodes(), scale=1, center=(0.5, 0.5), dim=2, resolution=np.pi / 16, equidistant=False
    )
    if labels:
        nx.draw(
            G,
            pos,
            edge_color=colors,
            node_size=np.arange(1, G.number_of_nodes() + 1) / G.number_of_nodes() * 100,
