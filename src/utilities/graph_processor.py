...
def assign_branch(branch_dict, branch_arrangement, max_depth, root_depth, parent_root_depth = 0):
    """ 
    Arg requirements:
    ...
    for child in branch_dict["children"]:
        assign_branch(branch_dict=child, branch_arrangement=branch_arrangement, max_depth=max_depth, 
                      root_depth=branch_dict["depth"], parent_root_depth=root_depth)


def generate_graph_data(tree: BlockScoreTree):
    """ dfd
        Returns: 
            ...
            slant: boolean indicating whether the connections from the branch to its parent
            }
    ...
    for i in range(len(remaining_branches)): #TODO figure out what this is doing and rewrite it better
        placed = []
        unplaced = []
    ...
    for branch in primary_branches:
        assign_branch(branch_dict=branch, branch_arrangement=branch_arrangement,
                                max_depth=max_depth, root_depth=0)
